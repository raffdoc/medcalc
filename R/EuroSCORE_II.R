#' EuroScore II is a mortality estimation model used in cardiovascular medicine, particularly in cardiac surgery.
#' @param x is datframe with all variables included in orginal score from 2008
#' @return returns additive score added othe data frame wiht new variabel of Logistic EuroScore II.
#' @export
...
EuroScoreLog.II  <- function(x,...) {
        ...
        for(i in seq(along=x$age)) { 
                if (x$age[i]< 60) { x$log.age[i] <- 0.0285181 } 
                else {x$log.age[i] <- 0.0285181*(x$age[i]-60)}
        }
        #sex variable
        x$log.sex <- NULL
        for (i in seq(along=x$sex)) {
                if(x$sex[i]==1){x$log.sex[i]<-0.2196434} 
                else {x$log.sex[i]<- 0}
        }
        ...
        for(i in seq(along=x$ri)) { 
                if (x$ri[i]==1) { x$log.ri[i] <- 0 } else { 
                        if (x$ri[i]==2) { x$log.ri[i] <- 0.303553 } 
                        else { 
                                if (x$ri[i]==3) { x$log.ri[i] <- 0.8592256 } 
                                else {x$log.ri[i] <- 0.6421508}
                        }
                }
        } 
        ...
        for (i in seq(along=x$eca)) {
                if(x$eca[i]==1){x$log.eca[i]<- 0.5360268} 
                else {x$log.eca[i]<- 0}
        }
        ...
        for (i in seq(along=x$ae)) {
                if(x$ae[i]==1){x$log.ae[i]<-0.6194522} 
                else {x$log.ae[i]<- 0}
        }
        ...
        for (i in seq(along=x$cps)) {
                if(x$cps[i]==1){x$log.cps[i]<-1.086517} 
                else {x$log.cps[i]<- 0}
        }
        # Diabetis on insulin therapy
        x$log.doi <- NULL
        for (i in seq(along=x$doi)) {
                if(x$doi[i]==1){x$log.doi[i]<-1.086517} 
                else {
                        x$log.doi[i]<- 0
                }
        }
        ...
        for(i in seq(along=x$nyha)) { 
                if (x$nyha[i]==1) { x$log.nyha[i] <- 0 } 
                else { 
                        if (x$nyha[i]==2) { x$log.nyha[i] <- 0.1070545 
                                            } 
                        else { 
                                if (x$nyha[i]==3) { x$log.nyha[i] <- 0.2958358 
                                } 
                                else {
                                        x$log.nyha[i] <- 0.5597929
                                }
                        }
                }
        } 
        ...
        for(i in seq(along=x$lv.ef)) { 
                if (x$lv.ef[i]>50) { x$log.lv.ef[i] <- 0 
                                     } 
                else { 
                        if (x$lv.ef[i]>31) { x$log.lv.ef[i] <- 0.3150652 
                                             } 
                        else { 
                                if (x$lv.ef[i]>21) { x$log.lv.ef[i] <- .8084096 } 
                                else {x$log.lv.ef[i] <- 0.9346919}
                        }
                }
        } 
        ...
        for(i in seq(along=x$ph)) { 
                if (x$ph[i]==1) { x$log.ph[i] <- 0 } 
                else {
                        if (x$ph[i]==2) {x$log.ph[i] <- 0.1788899 } 
                        else { x$log.ri[i] <- 0.3491475}
                }
        } 
        ...
        for(i in seq(along=x$em)) { 
                if (x$em[i]==1) { x$log.em[i] <- 0 } 
                else { 
                        if (x$em[i]==2) { x$log.em[i] <- 0.3174673 } 
                        else { 
                                if (x$em[i]==3) { x$log.em[i] <- 0.7039121 } 
                                else {x$log.em[i] <- 1.362947}
                        }}}
        ...
        for(i in seq(along=x$woi)) { 
                if (x$woi[i]==1) { x$log.woi[i] <- 0 } 
                else { 
                        if (x$woi[i]==2) { x$log.woi[i] <- 0.0062118 } 
                        else { 
                                if (x$woi[i]==3) { x$log.woi[i] <- 0.5521478 } 
                                else {x$log.woi[i] <- 0.9724533}
                        }
                }
        }
        ...
        for (i in seq(along=x$sta)) {
                if(x$sta[i]==1){x$log.sta[i]<-0.6527205 } 
                else {x$log.sta[i]<- 0}
        }
        ...
        for (i in seq(along=x$log.age)){
                ...
                )/(1+exp(-5.324537+x$log.age[i]+x$log.sex[i]+x$log.ri[i]+
                                 x$log.eca[i]+x$log.pm[i]+x$log.pcs[i]+
                                 x$log.cld[i]+x$log.ae[i]+x$log.cps[i]+
                                 x$log.doi[i]+x$log.nyha[i]+x$log.ccs[i]+
                                 x$log.lv.ef[i]+x$log.rmi[i]+x$log.ph[i]+
